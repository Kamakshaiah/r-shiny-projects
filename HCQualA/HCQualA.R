...
ui <- fluidPage(
  ...
  navbarPage(title = "HCQualA",
             tabPanel("Home",
                      jumbotron("Hi Welcome to HCQualA", paste("This is web application for quality people! Right now application supports DMAIC methodology and few Health Care Quality Models including META ANALYSIS."), buttonLabel = "Click Me" )
             ...
             navbarMenu("Statistical Analysis",
                        ...
                        tabPanel("Plots",
                                 sidebarLayout(
                                   sidebarPanel(
                                     radioButtons("plotoption", "Choose the Option:", choices = c("Histogram", "Bar Plot", "Box Plot", "Scatter", "Pie" )),
             ...
             navbarMenu("Models",
                        tabPanel("Donabedian Model",
                                 sidebarLayout(
                                   sidebarPanel(
                                     selectInput("dbmvar1", "Select Variable", choices = "", selected = ""),
                                     selectInput("dbmvar2", "Select Variables", choices = "", selected = "", multiple = TRUE),
                                     radioButtons("dbmoption", "Select Choice", choices = c("Cross Table", "Fit", "Fit Significance", "Exp. Coef.", "Plot")),
                                     hr(),
                                     helpText("Use 'donabedian.csv' for testing."),
                                     hr(),
                                     helpText("For details read:"),
                                     a(href="https://en.wikipedia.org/wiki/Donabedian_model", "Donabedian Model")
                                   ),
                                   mainPanel(
                                     div(verbatimTextOutput("dbmoutput")),
                                     div(plotOutput("dbmplot"))
                                   )
                                 )
                        ),
                        tabPanel("Iron Triangle Model",
                                 
                                 sidebarLayout(
                                   sidebarPanel(
                                     textInput("itnf", "No. of Factors", value = 3, placeholder = "Write No. of Factors Required"),
                                     radioButtons("itoption", "Select Option", choices = c("No Option", "Effects", "Factor Fit", "Omega Fit", "Factor Structure", "Omega Plot", "Triangle")),
                                     hr(),
                                     helpText("Use 'irontriangle_ordinal.csv' for testing."),
                                     hr(),
                                     helpText("For details read:"),
                                     a(href="https://en.wikipedia.org/wiki/Iron_Triangle_of_Health_Care", "Iron Triangle")  
                                   ),
                                   mainPanel(
                                     div(verbatimTextOutput("itoutput")),
                                     div(plotOutput("itplot"))
                                   )
                                   
                                 )
                        ), 
                        tabPanel("Quality Health Outcomes Model")
             ),
             
             navbarMenu("Clinical",
                        tabPanel("Randomized Control Trials (RCT)"),
                        tabPanel("Cross Sectional Studies"),
                        tabPanel("Cohort Studies"),
                        tabPanel("Systematic Reviews"),
                        tabPanel("Meta Analysis",
                                 sidebarLayout(
                                   sidebarPanel(
                                     # selectInput("mavar", "Select Exp. & Control Variables", choices = "", selected = "", multiple = TRUE),
                                     selectInput("masvar", "Select Study Variable", choices = "", selected = ""),
                                     radioButtons("metaoption", "Select Option", choices = c("No Option", "Fit", "Plot")),
                                     hr(),
                                     helpText("Fitting MA requires 6 variables; first three must belong to experimental group, and second three must belong to control (placebo) group. Each group requires number, center, location as variables. Use 'amlodipine.csv' from datasets for testing."),
                                     hr(),
                                     helpText("For more details read:"),
                                     a(href="https://en.wikipedia.org/wiki/Meta-analysis", "Meta Analysis")
                                   ), 
                                   mainPanel(
                                     div(verbatimTextOutput("metaoutput")),
                                     div(plotOutput("metaplot"))
                                   )
                                 )
                        )
             ),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    } else if(input$plotoption == "Bar Plot"){
      barplot(df[, input$cols6], xlab = input$xaxisname, ylab = input$yaxisname, main = input$title)
    } else if(input$plotoption == "Box Plot"){
      boxplot(df[, input$cols6], xlab = input$xaxisname, ylab = input$yaxisname, main = input$title)
  ...
  # output$occoutput <- renderPrint({
  #   if (input$occtype == "xbar"){
  #     occout()
  #   }
  # })
  ...
  observeEvent(input$file1,{
    updateSelectInput(session, inputId = "dbmvar1", choices = names(data_input()))
    updateSelectInput(session, inputId = "dbmvar2", choices = names(data_input()))
  }
  )
  ...
  dbmout <- reactive({
    ...
    if (input$dbmoption == "Cross Table"){
      # lapply(mydf[, c("structure", "process", "outcomes" )], table)
      out <- ftable(xtabs(~ df[, input$dbmvar1] + df[, input$dbmvar2], data = df))
      return(out)
    } 
    
    fit <- polr(as.formula(df[, input$dbmvar1] ~ .), data = df, Hess = TRUE)
    
    if (input$dbmoption == "Fit"){
      # fit <- polr(as.formula(df[, input$dbmvar1] ~ .), data = df, Hess = TRUE)
      return(summary(fit))
    } 
    
    if (input$dbmoption == "Fit Significance"){
      ctable <- coef(summary(fit))
      
      pvalues <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
      
      ctable <- cbind(ctable, pvalues)
      return(ctable)
      
    }
    
    if (input$dbmoption == "Exp. Coef."){
      ci <- confint(fit)
      expout <- exp(cbind(OR=coef(fit), ci))
      return(expout)
    }
    
    if (input$dbmoption == "Plot"){
      
      plot(df[, input$dbmvar1], fit$fitted.values)
    }
    
    
  })
  
  output$dbmoutput <- renderPrint({
    if (input$dbmoption == "Cross Table"){
      dbmout()
    } else if (input$dbmoption == "Fit"){
      dbmout()
    } else if (input$dbmoption == "Fit Significance"){
      dbmout()
    } else if(input$dbmoption == "Exp. Coef."){
      dbmout()
    } 
    
  })
  
  output$dbmplot <- renderPlot({
    if (input$dbmoption == "Plot"){
      dbmout()
    }
  })
  ...
  itout <- reactive({
    ...
    # lmfit <- lm(cost ~ quality + access, data = df)
        
    omegafit <- omega(df)
    
    fafit <- fa(cov(df), as.numeric(input$itnf))
    
    if (input$itoption == "Effects"){
      return(summary(lmfit))
    }
    
    if (input$itoption == "Factor Fit"){
      return(summary(fafit))
    }
    
    if (input$itoption == "Omega Fit"){
      return(omegafit)
    }
    
    if (input$itoption == "Factor Structure"){
      structure.diagram(fafit)
    }
    
    if (input$itoption == "Omega Plot"){
      omega.diagram(omegafit)
    }
    
    if (input$itoption == "Triangle"){
      
    }
    
  })
  
  output$itoutput <- renderPrint({
  ...
  observeEvent(input$file1,{
    updateSelectInput(session, inputId = "mavar", choices = names(data_input()))
    updateSelectInput(session, inputId = "masvar", choices = names(data_input()))
  }
  )
  ...
  metaout <- reactive({
    ...
    # out <- metacont(df[, "n.amlo"], df[, "mean.amlo"], df[, "var.amlo"],
    #                     df[, "n.plac"], df[, "mean.plac"], df[, "var.plac"],
    #                     data=df, studlab=df[, "study"])
    
    dfc <- data.frame(df[, input$mavar])
    
    out <- metacont(df[, 3], df[, 4], df[, 5], df[, 6], df[, 7], df[, 8], data=df, studlab=df[, input$masvar])
    
    if (input$metaoption == "Fit"){
      return(summary(out))
    }
    
    if (input$metaoption == "Plot"){
      forest(out)
    }
