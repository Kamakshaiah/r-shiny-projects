...
ui <- fluidPage(
  ...
  navbarPage(title = "RfQASS",
             ...
             navbarMenu("Models",
                        ...
                        tabPanel("Iron Triangle",
                                 
                                 sidebarLayout(
                                   sidebarPanel(
                                     textInput("efanf", "No. of Factors", value = 2, placeholder = "Write No. of Factors Required"),
                                     radioButtons("efaoption", "Select Option", choices = c("No Option", "Fit", "Plot")),
                                     hr(),
                                   ...
                                   mainPanel(
                                     div(verbatimTextOutput("efaoutput")),
                                     div(plotOutput("efaplot"))
                                   )
                        ...
                        navbarMenu("Clinical",
                                   tabPanel("Meta Analysis",
                                            sidebarLayout(
                                              sidebarPanel(
                                                selectInput("mavar", "Select Study Variable", choices = "", selected = "", multiple = TRUE),
                                                selectInput("masvar", "Select Study Variable", choices = "", selected = ""),
                                                radioButtons("metaoption", "Select Option", choices = c("Fit", "Plot")),
                                                hr(),
  ...
  server <- function(input, output, session) {
    ...
    omegaout <- reactive({
      ...
      if (input$efaoption == "Fit"){
        return(fit)
      }
      if (input$efaoption == "Plot"){
        omega.diagram(fit)
      }
    })
...
# QUALITY ANAYTICS
    ...
    dbmout <- reactive({
      ...
      if (input$dbmoption == "Plot"){
        ...
        plot(mydf[, input$dbmvar1], fit$fitted.values)
      }
    ...
    metaout <- reactive({
      ...
      out <- metacont(dfc[, 1], dfc[, 2], dfc[, 3], dfc[, 4], dfc[, 5], dfc[, 6], data=df, studlab=df[, input$masvar])
